...
$(document).ready(function(){  
    ...
    var config_text = $('#config-text');
    var config_apply_btn = $('#config-apply-btn');
    ...
    $('.redraw').on("input change", function () {
        if (warper) {
            warper.redraw();
        }
    });
    ...
    config_apply_btn.click(function() {
        if (warper) {
            warper.import(config_text.val());
        }
    });

    var on_redraw = function() {
        config_text.val(warper.export());
    };
        ...
        if (e.dataTransfer.files.length > 0) {
            ...
            reader.onload = function (event) {
                ...
                var img = render(event.target.result, function (imageData) {
                    ...
                    warper = new ImgWarper.PointDefiner(canvas, img, imageData, on_redraw);
                });
            };
